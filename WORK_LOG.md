...
- 2026-08-19, Decision, Full architecture overhaul planned — private Cloudflare app for a friend group, Ran a full extraction-system audit (SSRF/DNS-rebinding TOCTOU, isGenerated confidence-score bug, budget-guard pricing drift, JSON-LD-extracted-but-discarded bug, CLAUDE.md still describing Gemini after the OpenRouter migration), then gathered full v2 requirements (Google OAuth, shared/attributed recipe pool with adder-only edit rights, per-cook + aggregate ratings, recipe-level notes, tag-driven cuisine/food picker, restaurants cut, photo/OCR + PDF as new extraction sources, push + in-app notifications, installable PWA, clean-slate data) and wrote the full doc set: `REQUIREMENTS.md`, `docs/architecture/{README,stack-decision,data-model,extraction-pipeline,notifications,api-design}.md`, `docs/design/ux-flows.md`, `BUILD_GUIDE.md`. Updated `CLAUDE.md` and `.clinerules` to flag the repo as mid-overhaul and point at the new docs; added a pointer banner to `README.md`. No application code changed yet — v1 app is untouched and still live., Gives the project a durable, internally-consistent plan (stack decision, data model, extraction redesign, phased build order) to build against, and prevents agents/contributors from mixing v1 (public, cost-constrained, anonymous) and v2 (private, auth'd, cost-aware) assumptions mid-build.
...
- **2026-03-17**: Enhancement: permissive Nutrition Validation and KV Backoff Adjustments
  ...
  - **Description**: Addressed new edge cases in LLM outputs and persistent free-tier KV timeouts.
    ...
    - Increased KV timeouts to 4s and initial retry backoff to 1000ms to better accommodate Vercel free-tier cold starts.
