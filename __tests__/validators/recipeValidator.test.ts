import { validateRecipe, isValidRecipe } from '@/lib/validators/recipeValidator';
...
describe('recipeValidator', () => {
  ...
  describe('validateRecipe', () => {
    ...
    it('should reject recipe with empty ingredient item', () => {
      ...
      const result = validateRecipe(recipe);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.field === 'ingredients[1]')).toBe(true);
    });
    ...
    it('should reject recipe with empty instruction text', () => {
      ...
      const result = validateRecipe(recipe);
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.field === 'instructions[1]')).toBe(true);
    });
    ...
    it('should reject recipe with hallucination keywords in title', () => {
      const recipe = { ...validRecipe, title: 'Lorem ipsum placeholder recipe' };
      const result = validateRecipe(recipe);
      expect(result.isValid).toBe(false);
      expect(result.errors).toContainEqual({
        field: 'content',
    ...
    it('should accept recipe with optional fields omitted', () => {
      const recipe: Recipe = {
        ...
        instructions: [
          { step: 1, text: 'Mix flour and water' },
